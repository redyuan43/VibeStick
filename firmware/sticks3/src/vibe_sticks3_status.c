...
static bool s_ready;
static vibe_sticks3_status_t s_status;
...
static void status_style(vibe_sticks3_status_t status, const char **label,
                         uint16_t *background)
{
    switch (status) {
    case VIBE_STICKS3_STATUS_READY:
        *label = "READY";
        *background = 0x1565;
        return;
    case VIBE_STICKS3_STATUS_RECORDING:
        *label = "REC";
        *background = 0x0821;
        return;
    case VIBE_STICKS3_STATUS_SENDING:
        *label = "SEND";
        *background = 0xb26a;
        return;
    case VIBE_STICKS3_STATUS_DONE:
        *label = "DONE";
        *background = 0x168a;
        return;
    case VIBE_STICKS3_STATUS_ERROR:
        *label = "ERROR";
        *background = 0x7f1d;
        return;
    case VIBE_STICKS3_STATUS_WIFI:
    default:
        *label = "WIFI";
        *background = 0x3747;
        return;
...
static void render(vibe_sticks3_status_t status)
{
    static const uint8_t recording_heights[5] = {14, 22, 32, 22, 14};
    if (status == VIBE_STICKS3_STATUS_RECORDING) {
...
esp_err_t vibe_sticks3_status_init(void)
{
    ...
    ESP_RETURN_ON_FALSE(s_lock && s_transfer_done, ESP_ERR_NO_MEM,
                        "sticks3_status", "display synchronization");
    init_backlight();
    ...
    ESP_RETURN_ON_ERROR(
        ...
        "sticks3_status", "animation timer");
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_st7789(io, &panel_config, &s_panel),
                        "sticks3_status", "display panel");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_reset(s_panel), "sticks3_status",
                        "display reset");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_init(s_panel), "sticks3_status",
                        "display init");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_invert_color(s_panel, true),
                        "sticks3_status", "display invert");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_set_gap(s_panel, LCD_X_GAP, LCD_Y_GAP),
                        "sticks3_status", "display gap");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_disp_on_off(s_panel, true),
                        "sticks3_status", "display on");
    s_ready = true;
    s_status = VIBE_STICKS3_STATUS_ERROR;
