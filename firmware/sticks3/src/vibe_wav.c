...
bool vibe_wav_pcm_stream_info(const uint8_t *header,
                              ...
                              size_t *pcm_len)
{
    if (!header || header_len < 12 || total_len < header_len ||
        !pcm_offset || !pcm_len ||
    ...
    size_t offset = 12;
    bool format_ok = false;
    while (offset + 8 <= header_len) {
        const uint8_t *chunk = header + offset;
        const uint32_t chunk_size = read_le32(chunk + 4);
        const size_t data_offset = offset + 8;
        if (data_offset > total_len || chunk_size > total_len - data_offset) {
            return false;
        }
        if (memcmp(chunk, "fmt ", 4) == 0) {
            if (chunk_size < 16 || data_offset + 16 > header_len) {
                return false;
            }
            const uint16_t audio_format = read_le16(header + data_offset);
            const uint16_t channels = read_le16(header + data_offset + 2);
            const uint32_t sample_rate = read_le32(header + data_offset + 4);
            const uint16_t bits_per_sample = read_le16(header + data_offset + 14);
            format_ok = audio_format == 1 &&
                        channels == expected_channels &&
                        sample_rate == expected_sample_rate &&
                        bits_per_sample == expected_bits_per_sample;
        } else if (memcmp(chunk, "data", 4) == 0) {
            if (!format_ok || chunk_size == 0 || (chunk_size % 2) != 0) {
                return false;
            }
            *pcm_offset = data_offset;
            *pcm_len = chunk_size;
            return true;
        }

        const size_t padded_size = (size_t)chunk_size + (chunk_size & 1u);
        if (padded_size > total_len - data_offset) {
            return false;
        }
